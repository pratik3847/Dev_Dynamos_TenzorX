...
async def match_diagnosis(payload: DiagnosisMatchRequest) -> DiagnosisMatchResponse:
    ...
    # Hackathon Demo Fallback for common terms
    if not icd_results and not llm_result:
        lower_query = combined_query.lower()
        if "headache" in lower_query and "dizziness" in lower_query:
            icd_results = [Icd10Match(code="R51", name="Headache with Dizziness")]
        elif "headache" in lower_query:
            icd_results = [Icd10Match(code="R51", name="Headache")]
        elif "dizziness" in lower_query:
            icd_results = [Icd10Match(code="R42", name="Dizziness and giddiness")]
