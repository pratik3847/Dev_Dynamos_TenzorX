...
def login(payload: LoginRequest, db=Depends(get_db)) -> TokenResponse:
    try:
        with db.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                f"SELECT {USER_COLUMNS}, password FROM public.users WHERE email = %s",
                (payload.email.lower(),),
            )
            row = cur.fetchone()
...
def update_profile(
    ...
    set_clause = ", ".join([f"{col} = %s" for col in mapped.keys()])
    values = list(mapped.values()) + [str(current_user.user_id)]

    try:
        with db.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                f"UPDATE public.users SET {set_clause} WHERE user_id = %s RETURNING {USER_COLUMNS}",
                values,
            )
            row = cur.fetchone()
        if not row:
