...
export const StepHospitals = ({ specialty, city, coords, onNext }: Props) => {
  ...
  const withDistance = useMemo(() => {
    return all.map((h) => {
      const km = coords ? distanceKm(coords, { lat: h.lat, lng: h.lng }) : null;
      return {
        ...h,
        distanceKm: km,
  ...
  const [sortBy, setSortBy] = useState<SortKey>(coords ? "distance" : "rating");
    ...
    <div className="animate-slide-up space-y-6">
      <div className="text-center space-y-2 max-w-2xl mx-auto">
        ...
        <p className="text-muted-foreground">
          Showing {sorted.length} {specialty} hospitals {coords ? "near your live location" : `in ${city || "your area"}`} · Select up to 3 to compare ({selected.length}/3)
        </p>
        {coords && (
