...
export const StepInput = ({ onNext, initial, onCoords, coords }: Props) => {
  ...
  const [location, setLocation] = useState(initial?.location ?? "Mumbai");
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setLocation(stored.location ?? "Mumbai");
  ...
  }, [symptoms, diagnosis, age, budget, location, comorbidities]);
  ...
  const handleSubmit = () => {
    ...
    setLoading(true);
    setTimeout(() => onNext({ symptoms, diagnosis, age, budget, location, comorbidities }, coords ?? null), 700);
  };
  ...
  return (
    <div className="animate-slide-up space-y-6">
      ...
      <Card className="p-6 sm:p-8 shadow-elevated border border-border/60 bg-card rounded-3xl">
        ...
        <div className="mt-5">
          <p className="text-xs font-medium text-muted-foreground mb-2.5">Quick examples</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-7 pt-6 border-t border-border/60">
